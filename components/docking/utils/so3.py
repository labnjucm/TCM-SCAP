...
MIN_EPS, MAX_EPS, N_EPS = 0.0005, 4, 2000#控制截断无限级数计算的最小和最大误差范围。离散化的误差范围分割为 2000 个点。
X_N = 2000#用于角度离散化的采样点数量。
...
omegas = np.linspace(0, np.pi, X_N + 1)[1:]#从 0 到 π 均匀分布的角度样本（去除第一个点 0）。

#实现两个旋转向量的复合，先将旋转向量 r1 和 r2 转换为旋转矩阵，再相乘，最后转回旋转向量表示。
...
#在单位球面上生成一个随机向量，其模长由 sample 函数确定。
def sample_vec(eps):
    x = np.random.randn(3)# 生成一个三维随机向量，符合标准正态分布
    x /= np.linalg.norm(x) # 将向量归一化为单位长度
    return x * sample(eps)# 返回向量乘以通过 `sample` 函数得到的模长
...
def score_vec(eps, vec):
    ...
    om = np.linalg.norm(vec) # 计算输入向量的模长
    return np.interp(om, _omegas_array, _score_norms[eps_idx]) * vec / om

#获取分数的归一化值，用于进一步计算。
def score_norm(eps):
    eps = eps.numpy() # 将 `torch.Tensor` 转换为 NumPy 数组
    eps_idx = (np.log10(eps) - np.log10(MIN_EPS)) / (np.log10(MAX_EPS) - np.log10(MIN_EPS)) * N_EPS
    eps_idx = np.clip(np.around(eps_idx).astype(int), a_min=0, a_max=N_EPS-1)
