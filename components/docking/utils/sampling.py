...
#该函数用于对一组复合物进行位置随机化，并根据条件修改分子的构象（例如旋转和平移）。此函数支持随机化配体的位置、旋转角度以及部分受体的变动。
...
#该函数用于对分子进行采样，涉及到多个步骤，包括噪声扰动、模型推断和可能的可视化等。此函数主要用于在分子动力学模拟、生成模型（如扩散模型）等应用中，通过一系列步骤生成新的分子构象。
...
#该函数用于计算配体-受体复合物的亲和力。通常，亲和力指的是分子与目标（如蛋白质）的结合强度，在药物发现和分子对接中非常重要。此函数利用一个训练好的亲和力模型进行推理，并通过批处理进行加速。
