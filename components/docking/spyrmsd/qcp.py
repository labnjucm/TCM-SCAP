...
"""
该代码实现了用于快速计算分子间均方根偏差 (RMSD) 的两个关键矩阵：

M 矩阵：描述两个分子坐标矩阵的内积。
K 矩阵：对称关键矩阵，用于优化对齐分子时的旋转操作。
这些矩阵在基于四元数的方法中用于计算分子旋转对齐和 RMSD 值
"""
due.cite(
...
"""
该代码实现了计算分子结构中旋转对齐的核心步骤，通过四元数多项式计算分子旋转对齐时的最大特征值 λmax。主要功能包括:
计算四元数多项式的系数 c2、c1\c0。
通过优化或特征值分解找到最大根或最大特征值。
"""
#计算四元数多项式的系数
...
"""
该函数 qcp_rmsd 使用四元数多项式方法计算两个分子结构 A和 B之间的均方根偏差(RMSD)。四元数多项式方法是一种快速且精确的算法，用于处理分子坐标的旋转对齐和 RMSD 计算
"""
def qcp_rmsd(A: np.ndarray, B: np.ndarray, atol: float = 1e-9) -> float:
    ...
    ----------
    A: numpy.ndarray
        分子结构 A 的坐标矩阵，形状为 (N, 3)。
    B: numpy.ndarray
        分子结构 B 的坐标矩阵，形状为 (N, 3)。
    atol: float
        容差，用于数值误差的处理。
    ...
    注意
    -----
    如果 A 和 B 可以完全对齐（即只差异于质量中心的平移和旋转），
    我们期望 \( G_a + G_b = 2 \lambda_\text{max} \)。
    由于数值误差，可能出现 \( s = G_a + G_b - 2 \lambda_\text{max} \) 为负的情况，
    从而导致 \( \sqrt{s} \) 失败。在此情况下，当 \( |s| < atol \) 时，将 RMSD 设置为 0。
    """
