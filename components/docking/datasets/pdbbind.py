...
"""
用于向分子结构数据添加不同类型的噪声，包括平移、旋转和扭转噪声。
结合时间参数控制噪声分布，可以支持时间独立或依赖的噪声生成。
"""
class NoiseTransform(BaseTransform):
    ...
    #生成时间参数（t）以及其分量（t_tr, t_rot, t_tor）
    def get_time(self):
...
"""
用于加载和处理 PDBBind 数据集，包括配体和受体的图形表示、特征提取和缓存处理。
支持配体匹配、多构象生成、局部结构生成和嵌入加载
"""
...
#计算并打印分子复合物的统计信息，例如受体半径、分子半径、分子中心到受体的距离等
...
#从指定目录中读取单个分子的文件（支持 .sdf 和 .mol2 格式）
...
#从指定目录中读取所有分子文件（支持 .sdf 和 .mol2 格式）
