...
periodic_table = GetPeriodicTable()#使用 RDKit 获取周期表对象，用于获取元素的属性
allowable_features = {#定义了原子和残基可能的特征值范围
...
bonds = {BT.SINGLE: 0, BT.DOUBLE: 1, BT.TRIPLE: 2, BT.AROMATIC: 3}#定义键类型的映射关系
#计算分子和受体特征向量的维度，用于后续模型输入
...
"""
安全地获取元素 e 在列表 l 中的索引。
如果 e 不在列表中，返回列表的最后一个索引。
"""
def safe_index(l, e):
...
#从 PDB 文件中提取受体的序列、坐标以及相关特征，并构建复杂分子的图结构
...
#为给定的分子生成分子图，包括节点特征（原子特征）、边特征（键类型）以及坐标信息。
...
#为蛋白质受体中的原子生成特征，或生成代表未知原子的特征（misc 特征）
