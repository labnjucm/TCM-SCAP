...
"""
计算一个残基的化学基团(CG)与主链骨架(BB)原子之间的邻近残基数量。
仅计数与该残基的化学基团距离在 max_dist 范围内，且不属于 buffer_residue_num 范围内的残基。
"""
...
def identify_valid_vandermers(args):
    ...
    """
    """
    确定每个残基作为化学基团的可能性，并计算其与邻近残基的接触数量。
    最终返回一个张量，其中包含每个残基的接触数。
    """
...
"""
快速计算每个残基在序列中与其他残基的邻近关系。
考虑距离小于 max_dist 且不属于 buffer_residue_num 范围内的残基。
"""
...
#生成化学基团（CG）的特征张量，基于输入的氨基酸和其 SMILES 表达式
...
"""
该类用于加载、预处理和管理包含蛋白质侧链数据集的异构图数据。
提供了对侧链信息的提取和操作,包括计算范德华接触(van der Mer)等。

protein_graphs:存储所有加载的蛋白质图。
vandermers:存储范德华接触信息。
split_clusters:分割后保留的簇。
cluster_to_complexes:簇与蛋白质复合物的映射关系。
name_to_complex:蛋白质复合物名称与其异构图的映射。
"""
class PDBSidechain(Dataset):
    ...
    #根据分割类型（'train'、'val'、'test'）读取并过滤蛋白质簇信息
    def read_split(self):
        # read CSV file
