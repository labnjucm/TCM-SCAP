...
#用于将一个 Python 列表包装成符合 PyTorch 数据集接口的对象。
...
#读取模型参数配置文件 model_parameters.yml 并加载到命名空间对象 model_args 中
...
"""
定义了一个 ConfidenceDataset 类，用于加载、预处理和处理配体-受体复合物的数据集。
主要功能包括：
从缓存中加载或生成复合物图数据。
根据模型预测生成配体位置和 RMSD 值。
生成平衡的数据采样策略。
缓存和加载生成的配体位置和 RMSD 数据。
"""
class ConfidenceDataset(Dataset):
    def __init__(self, cache_path, original_model_dir, split, device, limit_complexes,
        ...
        """
        调用 get_args_and_cache_path 获取模型参数和缓存路径。
        如果未使用原始模型的缓存，则根据 args 和数据集划分生成新的缓存路径。
        """
        ...
        """
        检查是否已经生成了配体位置和 RMSD 数据。
        如果未生成，则调用 self.preprocessing 方法进行预处理
        """
        ...
        """
        加载生成的配体位置和 RMSD 数据。
        如果 cache_ids_to_combine 非空，则从多个缓存中合并数据
        """
    ...
    """
    初始化模型并设置推理调度。
    遍历所有复合物图，随机化配体位置并调用模型进行采样。
    计算 RMSD 并保存生成的配体位置和 RMSD 数据。
    """
